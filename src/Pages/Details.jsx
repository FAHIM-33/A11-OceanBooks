...
const Details = () => {
    ...
    return (
        <section className="max-w-7xl mx-auto my-8">
            <div className='bg-fadegray flex flex-col md:flex-row rounded-md overflow-hidden'>
                <div className=''>
                    <img src={img} className='h-[300px] mx-auto md:h-[90vh] block object-cover' alt="img of books" />
                </div>
                <div className='relative flex-1 flex flex-col justify-center p-4'>
                    <p className='mb-4 md:mb-8 rounded-sm w-fit bg-high text-background font-bold px-4 py-1'><BiCategoryAlt className='inline-flex mr-1 text-crim text-base md:text-xl'></BiCategoryAlt>{category}</p>
                    <p className='text-2xl md:text-4xl font-semibold border-l-2 border-crim pl-1'>{name}</p>
                    <p className='mb-4 text-sm mt-2'>by <span className='font-medium text-lg md:text-2xl'> {authorName}</span></p>
